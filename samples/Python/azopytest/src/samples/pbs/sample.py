...
class PbsTree:
    ...
    def recurse_print_result(self, result, node: PbsTreeNode):
        ...
        print(f"P{node.partID}: S{result.atoms[node.partID - 1][0] + 1}")
...
class PbsSample:
    ...
    def _load_tables(self):
        ...
        # load costs table
        self.costs_table = csvtable.CsvTable(os.path.join(self.db_path, "costs.csv"), True)

        # load pbs table
        self.pbs_table = csvtable.CsvTable(os.path.join(self.db_path, "pbs.csv"), True)
