...
if __name__ == "__main__":
    ...
    config.thread_count = int(args.tc)

    solver = azopy.Solver()
    result = solver.solve(config)

    # print sample problem specific results
    print("")
    ...
    match args.itest:
        case "nurse":
            ns.print_result(result)
        case "pbs":
            ps.pbs_tree.recurse_print_result(result, ps.pbs_tree.root)
    ...
    print("Atoms: ")
    print("[ ")
    for r in range(len(result.atoms)):
        res_row = result.atoms[r]
        print("\t", end="")
        print(res_row, end="")
        if r < len(result.atoms) - 1:
            print(",")
        else:
            print("")
